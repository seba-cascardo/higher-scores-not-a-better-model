...
\cref{disc:selector}) yet writing it back recovers little of the lift ($\Wknow$
$6.4\%$, a second correctness direction $20.35\%$, \cref{disc:geometry}) and the off-axis write that does lift never transfers back to
...
the best 1-D correctness discriminant recovers $6.4\%$ and a second, near-orthogonal
one $20.35\%$, and the trained offset has an
...
on-axis correctness direction is readable yet drives little of the lift
($\Wknow$ $6.4\%$, a second such direction $20.35\%$) while an off-axis one drives
all of it. A distinct mechanistic hypothesis for
