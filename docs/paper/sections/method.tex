...
\textbf{Scope of ``off-axis''.} Both references are \emph{one-dimensional per head}.
Everything we call off-axis is therefore off the measured one-dimensional linear
correctness direction, which is the strongest such direction we can build but not the
whole geometry of correctness: correctness information has been reported to occupy a
