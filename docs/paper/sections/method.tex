...
correctness-bearing subspace. We take one step into that gap rather than none: a
\emph{second} correctness direction --- a probe fitted on the scoring activations,
whitened cosine $0.101$ with $\Wknow$ --- injected the same way recovers $20.35\%$
of the lift where $\Wknow$ recovers $8.42\%$ on the same items
(\cref{disc:geometry}). The full $k$-dimensional test with norm-matched parallel and
residual re-injection remains the natural follow-up, and we do not claim its result
here; what we can say is that the increment from the first correctness direction to
the second is not zero. The on-axis
