...
All results are on Gemma~4 31B IT, run in bf16. The discrimination adapter (referred
to as V7-mc) is a contrastive-correctness PEFT in the LoFiT/ITI family: it learns
...
off-axis/on-axis split as a ratio relative to a random direction. The inducibility
control is a non-contrastive Align-LoRA: ordinary supervised fine-tuning on a
capacity- and budget-matched corpus, swept over LoRA rank (r8/32/64/256) and scored
through the same three-zone control gate, asking what fraction of the contrastive
lift generic fine-tuning recovers (this recovery fraction, the budget-matched
corpus, and the gate are all defined in \cref{sec:definitions}).
...
${\sim}62\%$ ARC recovery lands in the middle zone, and its delta-method fraction
interval $[45\%, 85\%]$ (\cref{disc:inducibility}) is what we report as ``zone C''
against these thresholds. A separate null-task check (MMLU) guards that a causal arm
