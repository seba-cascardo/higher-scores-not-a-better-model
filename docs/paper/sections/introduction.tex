...
Two facts follow at once. The lift is genuinely off the measured one-dimensional
correctness axis --- an on-axis push does nothing, and we mean by this the strongest
1-D linear correctness direction we can build per head, not the whole correctness
manifold (\cref{method:directions}) --- yet the \emph{learned} perpendicular direction is
