...
costs some $150\times$ more. The score moved; measurable capability did not.
...
\item \textbf{The read leg of read${\neq}$write (built on established setup, not a
  ...
  measured cost, though a reasoning
  test-time-compute baseline outscores both (\cref{disc:selector}). That a
...
independently trained adapters (\cref{tab:seeds}). The \emph{effect} and its off-axis
