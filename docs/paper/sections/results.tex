...
recovers and the $50\%$ mark --- so this null does exclude an effect \emph{along
$\Wknow$} of any magnitude we report as real, and only along it: a second on-axis
direction does write, at $20.35\%$ (below). It does \emph{not} license reading $\Wknow$ as
...
\textbf{upper bound}, not a zero. The estimation-side bound is the paired-bootstrap
ceiling already reported ($+13.6\%$); the MDE ($12.6\%$) is the design-side
statement --- the smallest effect this design would have caught at power $0.80$, not
a confidence limit --- and we cite it as that throughout. The two nearly coinciding
is the same precision seen twice, not a coincidence. This bound is on $\Wknow$: a second, near-orthogonal
...
is taken as the best 1-D on-axis reference.
...
Two things follow, and a third does not. First, the $\leq12.6\%$ figure above --- the
design-side bound --- is about $\Wknow$, not about ``on-axis'' as such. The clean form of that is the
head-to-head on the same $772$ items --- $20.35\%$ against $8.42\%$, intervals
disjoint --- rather than a comparison of the fitted direction's interval floor
($14.53\%$) against the MDE, which was computed for the $n{=}400$ design and so
describes a different item population. Where this paper says an on-axis
...
so the $6.4\%{\rightarrow}8.42\%$ difference is not protocol; what separates the two
figures is the item population they are scored on, the $400$ the probe was fitted on
against the $772$ held out from it. Third, and
...
$k$-dimensional subspaces remains open --- we injected the two directions one at a
time and never their span --- and this result is the reason it is worth running: the
increment from the first correctness direction to the second is not zero.
...
only 6.4\% for the on-axis $\Wknow$ reference, that reference's failure is a real
...
effect \textbf{exists and is paired-significant}, not that it is on-axis --- the best
1-D correctness discriminant recovers 6.4\% ($\Wknow$; a second correctness direction
reaches 20.35\%, \cref{disc:geometry}). \emph{Metric note:} the
...
best 1-D correctness discriminant ($\Wknow$, Fisher-LDA) & $\mathbf{6.4\%}$ (ARC null 0.500 / mc
