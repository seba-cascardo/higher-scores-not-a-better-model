...
\subsection{Most of the lift is generic to fine-tuning}\label{results:generic}

A matched non-contrastive Align-LoRA recovers ${\sim}62\%$ of the ARC lift, flat
across a rank sweep (\cref{tab:inducibility}: 59/63/60/65; slope-CI $[-1.4, +3.4]$
includes zero, so the residual is not a closeable capacity gap), placing most of the
effect on fine-tuning in general rather than on the contrastive objective; the
contrastive-specific remainder is at most ${\sim}38\%$ and is an upper bound: a
best-effort fairness control \textbf{ran} and does \textbf{not} raise the
${\sim}62\%$ --- increasing rank to r512 recovers \emph{less} (frac 0.22 at lr1e-4,
converged but overfitting) or yields a below-base broken adapter (lr2e-4), so
${\sim}62\%$ is a \textbf{tight} lower bound and the ${\sim}38\%$ remainder a firm
upper bound. TruthfulQA is excluded from this control as train-on-eval leakage
(held-out, the highest-rank control collapses $98\%{\rightarrow}30\%$).
...
condition. \textbf{Direction (loss) is robust; the canonical magnitude is now
...
multi-step-reasoning breakage, not truncation). The retrieval-side \textbf{lambada}
...
--- a separate earlier run; the per-item arbiter run scores $140/198 = 0.7071$,
differing by one item and supporting the same conclusion &
...
\centering
\begin{threeparttable}
\caption{\textbf{The recovery is mostly generic to fine-tuning (Align-LoRA matched
control).} A non-contrastive SFT control of matched capacity/budget recovers most of
the lift $\rightarrow$ the effect is \textbf{not special to the contrastive
objective}. ARC is the clean test; TQA is excluded as train-on-eval leakage.}
...
Quantity & Value & Source artifact \\
\midrule
ARC lift recovered by matched Align-LoRA (rank sweep r8/32/64/256) & fraction
$\mathbf{59/63/60/65 = {\sim}62\%}$, flat & \code{msap-\allowbreak phase1-\allowbreak 20260624} (rank
sweep) \\
Capacity-resolvability of the residual & slope-CI $\mathbf{[-1.4, +3.4]}$ (includes
0); delta-method frac-CI ${\sim}\mathbf{[45\%, 85\%]}$ (zone C) & same \\
...
\textbf{does not raise ${\sim}62\%$} & \code{msap-\allowbreak align-\allowbreak fairness-\allowbreak 20260702} \\
Contrastive-specific residual & ${\sim}\mathbf{38\%}$ of the ARC lift
(= 100\% $-$ ${\sim}62\%$) & derived \\
TQA excluded --- leakage (proof) & frac 41/66/72/91 monotone${\uparrow}$; held-out
20\%: r256 $\mathbf{98\%{\rightarrow}30\%}$; r8=r256=0.524; contrastive generalizes
held-out (0.83) & same \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item \textbf{Note.} The ${\sim}62\%$ is a \textbf{tight lower bound}. The
Align-fairness best-effort control (r512 / LR-sweep / +epochs) \textbf{ran} and does
\textbf{not} raise the generic fraction --- more capacity recovers \emph{less} (frac
0.22 at lr1e-4, converged; below-base broken at lr2e-4) $\rightarrow$ the
${\sim}62\%$ is the ceiling of generic-FT capacity and the ${\sim}38\%$ residual a
firm upper bound. This de-risk is now \textbf{closed}.
...
matters, the fraction interval is cross-checked by an independent
bootstrap-of-the-ratio (200k parametric draws from the published arm accuracies at
$n{=}400$, independent-binomial resampling --- conservative relative to the paired
design, whose positive inter-arm correlation would only tighten it): r256 frac CI
...
reading (\code{runs/\allowbreak oq1\_\allowbreak functional\_\allowbreak axis/\allowbreak frac\_\allowbreak ci\_\allowbreak bootstrap.\allowbreak json}). The r512 flank
...
\begin{figure}[t]
\centering
\includegraphics[width=.78\textwidth,alt={Scatter plot of the matched-control
...
recovery, generic SFT near 62 percent, contrastive adapter at 100 percent with low
cosine 0.088, and the on-axis reference W_know at high cosine 0.379 but only 6.4
percent recovery.}]{figures/fig_geometry_spectrum.pdf}
...
correctness direction $\vinf$ (net effect on activations for the three methods; the
...
recovers only 6.4\%, while the top recoverer (the contrastive adapter, 100\%) is
near-off-axis (cos 0.088). Most of the lift is generic to fine-tuning (generic SFT
${\sim}62\%$), and where the recovery lives is a method-dependent degree of
off-axis, not a single fact. Horizontal bars are the published 95\% intervals on the
recovery fraction where one exists --- $\Wknow$ $[-0.9\%, +13.6\%]$ and generic SFT
$[45\%, 85\%]$; the random floor (three seeds) and the contrastive anchor (100\% by
construction) carry none, and we do not synthesise one. \textbf{$\Wknow$'s interval
crosses zero and overlaps the random floor:} its 6.4\% is statistically
indistinguishable from no recovery (exact McNemar $p=0.136$), so the spectrum is
three rungs --- floor $\rightarrow$ generic SFT $\rightarrow$ contrastive --- not
four. The generic-SFT interval is correspondingly wide, which is why the claim it
supports is \emph{most of the lift}, not a specific percentage. Values in
\cref{tab:spectrum}.}
\label{fig:spectrum}
\end{figure}
...
\textbf{(1.74)}; direct $\thetamc$ offset cos 0.0014 & \code{msap-\allowbreak phase1-\allowbreak 20260624}
\code{derisk/\allowbreak contrastive\_\allowbreak direction} \\
generic SFT (net effect) & ${\sim}\mathbf{62\%}$ & $\mathbf{0.199}$
\textbf{(3.93)} & \code{msap-\allowbreak phase1-\allowbreak 20260624} \code{derisk/\allowbreak align\_\allowbreak direction} \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item \textbf{Verdict.} Both methods are \textbf{mostly off-axis} (cos${<}0.2$,
${\sim}$78--85$^{\circ}$ from $\vinf$), but generic SFT is $\mathbf{2.3\times}$
\textbf{more on-axis} than the contrastive adapter $\rightarrow$ \textbf{off-axis of
degree, not universal; not contrast-special.} The direct $\thetamc$ offset (cos
0.0014, a weight-space proxy) was misleading; the clean net-activation-effect
experiment is the canonical measurement. \textbf{Forward filter:} a zero-cost
read-space preview said ``off-axis'' (ratio ${\sim}1.05$); the clean experiment
...
$\mathbf{0.11}$ (MC $+35.0$ / VINF $+4.0$; $z = -6.63$; acc\_vinf 0.545, Wilson CI
$[0.496, 0.593]$ includes chance 0.500) & \code{runs/\allowbreak vinf\_\allowbreak causal/\allowbreak } +
...
\code{theta\_\allowbreak mc\_\allowbreak characterization.\allowbreak json} \\
