...
$1000/1000$ in every arm, and the lift is $+12.20$ under \code{acc} and
$\mathbf{+15.00}$ CI $[+11.70, +18.20]$ under \code{acc\_norm}. The two
...
quarter, consistent with the task heterogeneity already on record (Winogrande $79.5$
/ HellaSwag $69.5$ / ARC $36.2$ / TruthfulQA $0.9$; \cref{results:generic}).
...
therefore does not govern whether the exploit fires; distance from the distribution
the adapter was trained on does. That is the same conclusion \cref{disc:arbiter}
...
\textbf{0/1000} for the base against $146$ for the adapter and $298$ for plain CE ---
an ordering that replicates on a second benchmark (\cref{disc:pathologies}).

Two caveats bind this section. It is one run per cell. And it is not the converse arm
of \cref{lim:anchors}: the adapter here was trained on the same corpus as everywhere
