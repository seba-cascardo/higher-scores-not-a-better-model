...
cross-task \emph{ordering} of inducibility is not, and no mechanistic claim rests on
it) ---
...
GPQA out at CoT $0.70$ --- single-run, directional, and a floor, since $31/198$ traces
did not parse and are counted wrong). A per-item arbiter bounds it: the base
...
this size can be an off-axis, non-transferring readout edit --- matched by a read-only
selector at the same cost, and \emph{strictly dominated} by the most trivial baseline
...
adapter's $0.757$ --- $0.980$ against $0.855$ across all $400$ items --- at
\emph{half} the adapter's cost ($181$\,ms vs $354$\,ms, since
