...
recovers $36.2\%$ of the ARC lift (sd $5.0$ over three training seeds;
item-sampling uncertainty not included), while a non-contrastive LoRA with
...
r512 flank at ${\sim}117{,}000\times$ does not improve on it and can fall below
base). The two bars quantify disjoint uncertainties --- training runs and item
sampling --- and we do not test the difference between the two fractions.
...
$\rightarrow$ contrastive $100\%$ --- with the best 1-D correctness discriminant
($\Wknow$) at $6.4\%$, \emph{statistically indistinguishable from zero} (exact
McNemar $p=0.136$), counted with the floor rather than as a rung; a second,
near-orthogonal correctness direction recovers $20.35\%$, so the on-axis floor is
one direction's, not the correctness subspace's. To our knowledge no prior work runs matched
controls and reports these \emph{fractions}; that attribution is the load-bearing
novelty.

The contrastive adapter's write is \emph{causally off-axis of degree} --- off the
measured linear
...
distribution covers them --- only the covered one lifts, a protocol-bound inference:
benchmark identity carries format, domain and option count with it
(\cref{results:mmlupro}). Practically, this
is a caution for the contrastive-PEFT-for-multiple-choice program, drawn from one
adapter of that family rather than from its published recipes: a cold-MC lift of
