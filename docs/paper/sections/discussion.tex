...
\subsection{The recovery is mostly generic to fine-tuning, not special to the
contrastive objective}\label{disc:inducibility}

If the lift were a specific consequence of the \emph{contrastive-correctness}
objective, an ordinary supervised fine-tune of matched capacity and budget should
not reproduce it. It largely does. We train a non-contrastive Align-LoRA on the same
data budget and run it through the same causal recovery machinery (a sign-safe
three-zone control gate). It recovers about \textbf{62\%} of the ARC lift, and that
fraction is \textbf{flat across a rank sweep} (r8/32/64/256: 59/63/60/65), with the
sweep slope's confidence interval spanning zero ($[-1.4, +3.4]$) and a delta-method
fraction interval of roughly $[45\%, 85\%]$ (zone C). The residual is therefore not
a capacity gap the generic control can close with more parameters; ordinary
fine-tuning simply buys back most of what cold scoring leaves on the table. One
caveat we apply to ourselves, symmetric with the information-set discipline of
\cref{disc:channel}: ``matched capacity and budget'' matches examples and optimizer
steps, not the effective training signal per step --- the contrastive objective sees
each example with an explicit correct/incorrect contrast where the SFT objective
sees only the correct completion --- so the ${\sim}62\%$ attributes the lift to
\emph{fine-tuning on this data} rather than to any stronger claim of per-step
informational equivalence between the two objectives. The contrastive-specific
remainder is at most ${\sim}38\%$ of the ARC lift, and this is a \textbf{firm upper
bound}: a best-effort fairness control has now run and does \textbf{not} raise the
generic fraction --- raising the rank to r512 recovers \emph{less} (frac 0.22 at
lr1e-4, converged but overfitting ep2${\rightarrow}$ep4, robust ep2=ep4) or yields a
below-base broken adapter (lr2e-4, excluded like the r256hi high-LR control), so the
${\sim}62\%$ is the ceiling of generic-fine-tuning capacity, a \textbf{tight} lower
bound. That de-risk (higher rank, an LR sweep, more epochs) is now closed; it
refined the residual, not the thesis.
...
124/124 under CoT) suggests, and against the trivial one-forward baseline the review
asked for --- is settled below by the deployment triangle: it does \textbf{not}. A
reasoning TTC ties the cheapest baseline, and a single-forward letter-emission of
the base dominates the selector itself at equal cost --- which is exactly why we
hold the selector to its \emph{evidentiary} role (read${\neq}$write) rather than a
deployment win.
...
198 base-wrong ARC items, four deployment routes separate on the (accuracy, cost)
plane. Three cost a single forward: the contrastive adapter's cold-MC scoring
recovers \textbf{0.763}; the read-only selector recovers \textbf{0.914}; and ---
the same-cost baseline the review asked us to run --- prompting the base to emit
the answer letter in one greedy forward, no chain of thought, recovers
\textbf{0.970} (192/198; 2 items unparsed and counted wrong, so a floor). The
fourth, a reasoning test-time-compute baseline (base self-consistency CoT, $k{=}3$),
also recovers \textbf{0.970}, but at ${\sim}1179$ generated tokens. So the single
cheapest intervention --- one forward, letter emission, no adapter and no probe ---
matches the expensive reasoning baseline and dominates both the adapter and the
selector at equal cost.
...
compares two readouts of the \emph{same base} at the \emph{same cost} --- loglik
continuation ranking vs letter emission --- which move it from 0.505 to 0.980 over
the full set ($0{\rightarrow}0.970$ on the base-wrong subset the triangle uses).
This is the paper's thesis, that the cold-MC channel is lossy, in its sharpest
form: the knowledge is present and cheaply reachable by a different readout of the
base, and the adapter's off-axis write recovers \emph{less} of it (0.763) than the
most trivial same-cost alternative (0.970).

\begin{figure}[t]
\centering
\includegraphics[width=.78\textwidth,alt={Scatter plot of the deployment triangle
on the 198 base-wrong ARC items: x-axis is cost per item in generated tokens on a
log scale, y-axis is ARC accuracy. The contrastive adapter sits at 0.763 for one
forward, the read-only selector at 0.914 for one forward, base letter-emission at
0.970 for one forward, and base self-consistency chain-of-thought at 0.970 at about
1179 tokens.}]{figures/fig_deployment_triangle.pdf}
\caption{\textbf{The deployment triangle on the 198 base-wrong ARC items.} $x$:
cost per item in generated tokens (log scale; a single forward is one unit); $y$:
ARC accuracy. Four routes: the contrastive adapter's cold-MC scoring (0.763, one
forward) is dominated by everything; the read-only selector (0.914, one forward)
improves on it read-only; \textbf{the base asked to emit the answer letter in one
forward (0.970) matches the reasoning baseline at ${\sim}1000\times$ less cost and
dominates both the adapter and the selector at equal cost}; base self-consistency
CoT (0.970, ${\sim}1179$ tokens) ties letter-emission at far higher cost. The whole
LoFiT/ITI-for-cold-MC family optimizes a readout that the most trivial same-cost
alternative --- ask the base for the letter --- already beats. \textbf{The
comparison is evidentiary, not a like-for-like scoring test:} letter emission is a
different (lettered-MCQ) readout than the cold-MC channel
(\cref{method:protocol}), so the triangle contrasts two readouts of the same base
at equal cost --- the lossy-readout thesis in its sharpest form --- and the
selector's role stays evidentiary (read${\neq}$write), not a deployment
recommendation, since even it is dominated by letter emission.}
\label{fig:triangle}
\end{figure}
...
\caption{\textbf{Deployment routes on the 198 base-wrong ARC items} (the numeric
view of \cref{fig:triangle}).}
\label{tab:triangle}
\footnotesize
\begin{tabular}{L{4.2cm} L{4.2cm} L{2.8cm} L{3.2cm}}
\toprule
...
unparsed as wrong $\rightarrow$ floor) & 1 forward (a few decode tokens) &
\textbf{matches SC-CoT; dominates the selector and adapter at equal cost} \\
Base self-consistency CoT ($k{=}3$) & \textbf{0.970} (192/198; $k{=}5$ identical
$\rightarrow$ saturates at $k{=}3$) & ${\sim}1179$ generated tokens & ties
letter-emission at ${\sim}1000\times$ the cost \\
Read-only selector (\cref{disc:selector}) & \textbf{0.914} & 1 forward
(${\sim}1000\times$ cheaper than CoT) & dominated by letter-emission;
Pareto-dominates only the \emph{adapter} \\
Contrastive adapter (cold-MC scoring) & \textbf{0.763} & 1 forward & dominated by
all three \\
\bottomrule
\end{tabular}
\end{threeparttable}
...
a deployment win (dominated at equal cost by letter emission). \emph{Population
note (do not conflate):} the four figures here (letter-emission 0.970, SC-CoT
0.970, selector 0.914, adapter 0.763) are all on the \textbf{198 base-wrong}
...
population and are not directly comparable to the 0.914 / 0.763 here. The 0.943 and
0.914 are the same selector on two populations, not two systems. ``All items'' here
...
decisive contrast is with the generic control: a capacity-matched Align-LoRA, which
...
``single-model'' from the effect; its off-axis geometry replicates too (whitened
cos $-0.002$, frac${\perp}$ 97.5\%; below).
...
Items resolved this pass: the canonical gsm8k loss magnitude is $\mathbf{-25\pp}$
...
reasoning breakage) and not truncation (\cref{disc:pathologies}); the
temperature/null-space negative is now \textbf{causal} (patching, temperature
fraction 0.215 at scoring positions --- \cref{disc:negatives}); and the effect
replicates cross-family (Qwen 14B, 4/4-sign lift). Items still pending, marked here
rather than asserted: the ${\sim}62\%$ generic fraction is a \textbf{tight lower
bound} --- the Align-LoRA best-effort fairness control ran (r512 recovers
\emph{less}, not more, so the contrastive-specific residual ${\sim}38\%$ is a firm
upper bound); the present-but-swamped claim's base-arm residual CI lower bound
...
replicates too (Qwen whitened $\cos(\thetamc, \vinf)$ $-0.002$ within null,
perpendicular fraction 97.5\%; above). The collateral reasoning loss is now shown
