...
assert that the adapter's off-axis direction aligns with $\vret$. We do not resolve
that either way: we measure $\coswh(\thetamc,\vinf)$ and $\coswh(\vinf,\vret)$, but
never $\coswh(\thetamc,\vret)$, so whether this spillover is an off-axis
perturbation of a substrate the two faces share, or the lift partly projecting onto
the retrieval face, is open. The pathologies are the downstream
