...
\paragraph{The response is not a graded function of projection onto the learned
...
(\code{acc\_norm}, $n{=}1172$, paired) gives: $\thetamc$ at $+1$, $+36.26\pp$ CI
$[+33.28, +39.25]$; and for a separately trained anti-axis, $-0.5 \rightarrow -1.96$,
...
side of zero, so the dial is unilaterally negative and the response is not graded in
its projection onto $\thetamc$. The rank data say the same thing qualitatively: of
...
zero is not a near-miss. Sweeping the cutoff from $0.20$ to $0.60$ moves one arm's
count from $147$ to $156$: the split is categorical, not a knob. And the degenerate
traces are what exhausts the generation budget rather than long reasoning ---
median lengths of $23$--$54$ tokens with $13$--$34\%$ of traces at the cap is a
bimodal distribution, and $311$ of one arm's $397$ unparsed traces are loops.
...
sizes. One run per cell throughout.
...
$2560$ tokens it generates \emph{more} than the base ($1488$ against $1398$ on the
