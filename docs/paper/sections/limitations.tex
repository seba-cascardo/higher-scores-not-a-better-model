...
of inducibility, which is an acc\_norm statement and is cited as such throughout, not
as a property of the models.
...
\paragraph{``Generic'' versus ``domain exposure'': resolved --- any fine-tuning.}
...
readings: that ordinary fine-tuning of \emph{any} kind recovers most of the lift, or
that fine-tuning \emph{on this domain} does. We ran the missing arm. Holding
parameterization, budget and schedule fixed and swapping only the corpus for MedMCQA
...
checkpoint and $\mathbf{1.00}$ at the final one. \textbf{The domain is not what
matters:} plain cross-entropy trained on medicine buys back as much of an ARC lift as
plain cross-entropy trained on ARC itself.
...
domain shift is a real axis at $+0.90 \pm 0.17$ ($5.4\sigma$), but it differs from the
...
lift, and the same non-transfer to generation ($-49$ pp on gsm8k, turn termination
