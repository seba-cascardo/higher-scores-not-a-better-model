...
correctness direction recovers $20.35\%$, clearing both that design-side figure and
the $+13.6\%$ bootstrap ceiling on $\Wknow$'s own recovery (\cref{disc:geometry}). The full-set $\Delta$ is now confirmed (the v7mc full-set
...
\paragraph{The strongest 1-D on-axis arm has now run; the $k$-dimensional test has
not.}
The on-axis references we inject are constructed, and \cref{disc:geometry} shows
...
better on-axis direction exists. There is a principled candidate, and it has now been
run: the decision direction of the probe \emph{fitted} on these activations, the
empirically optimal linear correctness reader in this space, injected norm-matched
through the identical harness (maximum per-head deviation $4.73\times10^{-7}$). It
recovers $20.35\%$ of the lift on the $772$ items held out from its own fit, against
$\Wknow$'s $8.42\%$ on the same items, intervals disjoint (\cref{disc:geometry}) --- so
the on-axis floor this paper reports belongs to the best 1-D discriminant and not to
correctness as such, and every on-axis figure we quote carries that qualifier. One
design constraint was load-bearing and is recorded in the blob's own configuration
rather than left to the person who runs it: unlike $\vinf$ and $\Wknow$, which are
estimated on a separate corpus, the probe is fitted \emph{on ARC}, on items
$0$--$399$, so its evaluation scores the complement, $400$--$1171$; out of sample it
still reads gold-vs-distractor at $0.9486$ (CI95 $[0.9318, 0.9637]$).
What is \emph{not} done is the $k$-dimensional test. We injected a second direction on
its own, never the two together, and never a norm-matched projection onto their span
with the residual re-injected against it. So what we can say is that the increment
from the first correctness direction to the second is not zero; we cannot say what a
correctness \emph{subspace} of rank two to eight recovers, and nothing we report
bounds it. That test needs the GPU class this arm needed, and it is the natural
follow-up.
