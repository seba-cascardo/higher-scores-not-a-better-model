...
collapses $98\%{\rightarrow}30\%$), so its generic-fine-tuning claim is an ARC result.
...
$+19.08 \pm 0.88$), the off-axis net-effect ratio is $\mathbf{1.85 \pm 0.13}$ against
...
outlier favoured by the earlier construction.
...
$\mathbf{-24.2}$ & $\mathbf{1.85}$ & --- \\
 & $\pm 0.63$ & $\pm 1.13$ & $\pm 1.38$ & $\pm 0.88$ & $\pm 7.3$ & $\pm 0.13$ & \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item \textbf{Reading.} The scoring lift and the off-axis geometry are stable
(ARC sd $0.63$\,pp; ratio sd $0.13$); the generation loss is not (range $-16.5$ to
...
(\cref{disc:pathologies}). Artifacts: \code{runs/\allowbreak v7\_\allowbreak lofit\_\allowbreak seed\{1,2\}/}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
items, dominating both the read-only selector (0.911) and the adapter's cold-MC
scoring (0.757). And the cheapest baseline available --- the base emitting the answer
...
${\sim}150\times$ less cost while \emph{strictly dominating} the selector and the
adapter, at roughly half their cost (181\,ms against 360 and 354\,ms, since cold-MC
scoring runs one forward per option). So the adapter loses to a reasoning baseline
\emph{and} to the most trivial alternative on both axes at once, and the selector's
value is evidentiary, not deployment (it too is dominated by letter emission). The one scope caveat is that letter emission
